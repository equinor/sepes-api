...
namespace Sepes.Tests.Util
{
    public class StudyAccessUtilTest_SingleStudy : StudyAccessUtilTest_Base
    {
        ...
        {
            var db = GetContextWithSimpleTestData(UserConstants.COMMON_USER_ID, COMMON_STUDY_ID, false);
            
            var userServiceMock = UserFactory.GetUserServiceMockForBasicUser(true, UserConstants.COMMON_USER_ID);        
        ...
        public async void ReadingUnrestrictedStudyAsEmployee_ShouldThrow()
        {
            ...
            var userServiceMock = UserFactory.GetUserServiceMockForBasicUser(false, UserConstants.COMMON_USER_ID);
        ...
        {
            var db = GetContextWithSimpleTestData(UserConstants.COMMON_USER_ID, COMMON_STUDY_ID, true);

            var userServiceMock = UserFactory.GetUserServiceMockForAdmin(UserConstants.COMMON_USER_ID);
        ...
        {
            var db = GetContextWithSimpleTestData(UserConstants.COMMON_USER_ID, COMMON_STUDY_ID, true, studySpecificRole);

            var userServiceMock = UserFactory.GetUserServiceMockForAppRole(appRole, UserConstants.COMMON_USER_ID);
        ...
        public async void ReadingRestrictedStudyWithRelevantPermission_ShouldSucceed(bool employee, string studySpecificRole)
        {
            ...
            var userServiceMock = UserFactory.GetUserServiceMockForBasicUser(employee, UserConstants.COMMON_USER_ID);
            var study = await StudySingularQueries.GetStudyByIdCheckAccessOrThrow(db, userServiceMock.Object, COMMON_STUDY_ID, UserOperation.Study_Read);
        ...
        {
            var db = GetContextWithSimpleTestData(UserConstants.COMMON_USER_ID, COMMON_STUDY_ID, true, justSomeBogusRole);
